...
OPENML_DATACONFIG = {
    ...
    'adult':{'num':['age', 'fnlwgt', 'education-num']},
    'cylinder-bands': {'bin':['grain_screened', 'proof_on_ctd_ink', 'blade_mfg', 
                            'direct_steam', 'type_on_cylinder', 'plating_tank']},
    'telco-customer-churn': {'bin':['Partner', 'Dependents', 'PhoneService', 'PaperlessBilling'], 
                             'cat':['gender', 'MultipleLines', 'InternetService', 
                                     'OnlineSecurity', 'OnlineBackup', 'DeviceProtection', 'TechSupport',
                                     'StreamingTV', 'StreamingMovies', 'Contract', 'PaymentMethod', 'TotalCharges',
                                     ]},
    'Bank_marketing_data_set_UCI': {'bin':['default', 'loan'],
...
def load_data(dataname, dataset_config=None, encode_cat=False, data_cut=None, seed=42):
...
def load_single_data(dataname, dataset_config=None, encode_cat=False, data_cut=None, seed=123):
    ...
    if os.path.exists(dataname):
        ...
        # If dataset is retrieved by name, a version may be specified. If no version is specified and multiple versions of the dataset exist, the earliest version of the dataset that is still active will be returned.
        ...
        # 对bin特征列要单独处理，从cat中抽离出来
        bin_cols = []
        if dataname in dataset_config:
            if 'bin' in dataset_config[dataname]: 
                bin_cols = [c for c in all_cols if c in dataset_config[dataname]['bin']]

            if 'cat' in dataset_config[dataname]:
                cat_cols = [c for c in all_cols if c in dataset_config[dataname]['cat']]
        
        cat_cols = [c for c in cat_cols if c not in bin_cols]
        num_cols = [c for c in num_cols if c not in bin_cols and c not in cat_cols]

        if dataname in dataset_config and 'num' in dataset_config[dataname]:
            num_cols = [c for c in all_cols if c in dataset_config[dataname]['num']]
            cat_cols = [c for c in cat_cols if c not in bin_cols and c not in num_cols]
    ...
    #     'data/IC',
    #     ]
    # if dataname in big_data:
    #     X = pd.concat((X,y), axis=1)
    #     # 这里没有分层抽样,但好像随机的效果基本差不多
